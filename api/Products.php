...
class Products extends Okay {
    ...
    public function get_products($filter = array()) {
        ...
        if(!empty($filter['variant_id'])) {
            $variant_id_filter = $this->db->placehold('AND pv.id in(?@)', (array)$filter['variant_id']);
            $variant_join = 'LEFT JOIN __variants pv ON pv.product_id = p.id';
        }
        ...
        foreach ($products as $product){
            $product->category_url = $this->categories->get_product_categories_url($product->id);
            $product->category_id = $this->categories->get_product_categories_id($product->id);
            $product->url = $product->category_url.$product->url.".html";
        }
