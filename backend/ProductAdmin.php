...
class ProductAdmin extends Okay {
    ...
    public function fetch() {
        ...
        if($this->request->method('post') && !empty($_POST)) {
            ...
            $product->external_id = $this->request->post('external_id');
            ...
            elseif(substr($product->url, -1) == '-' || substr($product->url, 0, 1) == '-') {
                ...
                if($product->id) {
                    ...
                    $variant_install_price = $this->request->post('variant_install_price');
                    if($this->request->post('variant_install_ot')) {
                        $variant_install_ot = $this->request->post('variant_install_ot');
                    }else{
                        $variant_install_ot = false;
                    }

                    if(!empty($variant_install_price)){
                        $this->variants_install->delete_variants_install_for_product($product->id);

                        $variant_install = new stdClass();
                        $variant_install->product_id = $product->id;
                        $variant_install->price = $variant_install_price;
                        $variant_install->price_ot = $variant_install_ot;
                        $this->variants_install->add_variant_install($variant_install);
                    }
                    ...
                    foreach ($current_files as $file) {
                        if (!is_array($files['id']) or !in_array($file->id, $files['id'])) {
            ...
            if($product) {
               ...
               //Вариант с установкой
                $variant_install = $this->variants_install->get_variants_install(array('product_id'=>$product->id));
                $variant_install = $variant_install[0];
        ...
        $this->design->assign('product_variant_install', $variant_install);
