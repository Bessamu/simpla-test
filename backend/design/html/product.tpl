...
{if $message_success}
    <div class="message message_success">
        ...
        <a class="link" target="_blank" href="../{$lang_link}catalog/{$product->url}">Открыть товар на сайте</a>
        {if $smarty.get.return}
        <a class="button" href="{$smarty.get.return}">Вернуться</a>
        {/if}

        <span class="share">
            <a href="#" onClick='window.open("http://vkontakte.ru/share.php?url={$config->root_url|urlencode}/catalog/{$product->url|urlencode}&title={$product->name|urlencode}&description={$product->annotation|urlencode}&image={$product_images.0->filename|resize:1000:1000|urlencode}&noparse=true","displayWindow","width=700,height=400,left=250,top=170,status=no,toolbar=no,menubar=no");return false;'>
            <img src="{$config->root_url}/backend/design/images/vk_icon.png" /></a>
            <a href="#" onClick='window.open("http://www.facebook.com/sharer.php?u={$config->root_url|urlencode}/catalog/{$product->url|urlencode}","displayWindow","width=700,height=400,left=250,top=170,status=no,toolbar=no,menubar=no");return false;'>
            <img src="{$config->root_url}/backend/design/images/facebook_icon.png" /></a>
            <a href="#" onClick='window.open("http://twitter.com/share?text={$product->name|urlencode}&url={$config->root_url|urlencode}/catalog/{$product->url|urlencode}&hashtags={$product->meta_keywords|replace:' ':''|urlencode}","displayWindow","width=700,height=400,left=250,top=170,status=no,toolbar=no,menubar=no");return false;'>
...
<form method=post id=product enctype="multipart/form-data">
	...
	<div id="external_id">
		...
		<input name="external_id" value="{$product->external_id}">
		<p>Если оставить пустым в RetailCRM будет передан id который присвоен на сайте</p>
	</div>
    ...
    <div id="variants_block"
		...
		<div id="variant_install_block">
			<label>Цена с установкой</label>
			<input type="text" name="variant_install_price" value="{$product_variant_install->price}">
			От
			<input type="checkbox" class="price_ot" name="variant_install_ot" value="1" {if $product_variant_install->price_ot}checked=""{/if}>
		</div>
        ...
        <div class="block layer">
            ...
            <ul>
                ...
                <li>
                    ...
                    <input name="url" class="page_url" type="text" value="{$product->orign_url|escape}"/></li>
                <li>
