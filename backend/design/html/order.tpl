...
<div id="name">
	...
	<input name=retail_id type="hidden" value="{$order->retail_id|escape}"/>
...
<div id="order_details">
	...
	<div style="margin-bottom: 10px">
		{if $order->retail_id}
			<label><a href="{$integration->urlRetail}/orders/{$order->retail_id}/edit" target="_blank">Номер заказа в RetailCRM: {$order->retail_id}A</a></label>
		{else}
			<label style="color: #ED145B;"><b>Заказа нет в RetailCRM</b></label>
        {/if}
	</div>
	<div id="user">
