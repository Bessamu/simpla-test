...
<input type=hidden name="session_id" value="{$smarty.session.id}">
	<div id="name">
		...
		<input name=retail_id type="hidden" value="{$user->retail_id|escape}"/>
		<div class="checkbox">
	...
	<!-- Левая колонка свойств товара -->
		...
		<div class="block">
			<ul>
				{if $user->retail_id}
				<li>
					<a href="{$integration->urlRetail}/customers/{$user->retail_id}" target="_blank">Id клиента в RetailCrm: {$user->retail_id}</a>
				</li>
                {else}
				<li>
					<p>Клиента нет в RetailCrm</p>
				</li>
                {/if}
				{if $groups}
				<li>
